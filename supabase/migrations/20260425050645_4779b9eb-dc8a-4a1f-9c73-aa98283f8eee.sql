-- Per-checkpoint scan history
CREATE TABLE public.ticket_scans (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  ticket_id UUID NOT NULL REFERENCES public.tickets(id) ON DELETE CASCADE,
  event_id UUID NOT NULL REFERENCES public.events(id) ON DELETE CASCADE,
  checkpoint_id UUID REFERENCES public.event_checkpoints(id) ON DELETE SET NULL,
  booth_id UUID REFERENCES public.event_booths(id) ON DELETE SET NULL,
  scanned_by UUID NOT NULL,
  scanned_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- A ticket can only check in once per checkpoint (NULL checkpoint = legacy single-gate)
CREATE UNIQUE INDEX ticket_scans_unique_per_checkpoint
  ON public.ticket_scans (ticket_id, COALESCE(checkpoint_id, '00000000-0000-0000-0000-000000000000'::uuid));

CREATE INDEX ticket_scans_event_idx ON public.ticket_scans (event_id, scanned_at DESC);

ALTER TABLE public.ticket_scans ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Scanners read event scans"
  ON public.ticket_scans FOR SELECT
  USING (public.can_scan_event(event_id, auth.uid()));

CREATE POLICY "Admins manage all scans"
  ON public.ticket_scans FOR ALL
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- Replace check-in function: per-checkpoint duplicate detection
CREATE OR REPLACE FUNCTION public.check_in_ticket(
  _qr_code text,
  _event_id uuid,
  _checkpoint_id uuid DEFAULT NULL,
  _booth_id uuid DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Per-checkpoint duplicate check
  ...
  -- Keep tickets table reflecting the FIRST check-in (back-compat)
